...
I'm only using data from clutches without a low nest sample size (LowNestSampleSize == 'No'), but I am retaining data from Season 1 since it should not affect how many males a female mates with (assuming all males are represented in all clutches of females they mate with) or the way that ranked males fertilize offspring in individual clutches. Season 1 data also do not seem to skew the remigration interval analysis in the way that we'd expect (i.e. to have migrating males that we'd miss observing), so we kept the partial Season 1 data for that analysis as well. 

I'd like to credit the folks who collected and started analyzing these data:

\textbf{Research leads:}
- Lisa Komoroske
- Mariana Fuentes
- Will White

\textbf{Field crew:}
- Armando Barsante Santos
- Cintia Miranda
- Blair Bentley
- Estefany Argueta
...
The goal of this analysis is to parameterize my model of sea turtle population dynamics by improving the reproductive demographic parameters, including the

Q1. distribution of the number of males that females mate with
Q2. the way that multiple males fertilize offspring in a clutch
Q3. the remigration interval for resident males

Basic overview of reproductive sub-model in my code:
...
For each breeding season, after survival and maturity, the number of adults that become available to breed are the sum of successful draws from a binomial distribution with probability equal to one over the remigration interval (Q3). For example, if there are 5 mature females with a remigration interval of 5 years, I will take 5 draws from a binomial distribution with probability of success equal to 1/5 = 0.2. 

Once I have the number of available males and females, I can calculate the operational sex ratio (number of males / number of all available adults), and use the mating function to translate that to the probability of females finding mates. 

The number of actually breeding females is then the sum of draws from a binomial distribution with the probability of success equal to the mating function output. For each female who breeds, she will mate with a certain number of males (Q1), to be drawn from a breeding pool, where each male is represented X times based on the minimum OSR for each mating function. For example, for the mating function with a minimum OSR of 0.05, each male can theoretically mate with 19 females, so there will be 19 copies of each male in the 'breeding pool', where the 'copies' are actually genotypes sampled from the mature male population genotypes. Maternal genotypes are similarly sampled from the mature female population genotypes. 

Then, she will lay a number of clutches and a number of eggs for each clutch drawn individually from the distributions of numbers of clutches per females and numbers of eggs per clutch from FdN data we collected 2020 - 2023. 
...
\includegraphics[width=1\textwidth]{C://Users/vique/Documents/Projects/iliketurtles3/figures/UpdatedMalePresence.png}
