...
So far, no more than one clutch is represented per female per season in the dataset. 
...
\textbf{Table 3. Model fit outputs.} 
\newline
...
$$ Proportion\ father\ contributes(Rank) = 0.687 \cdot Rank^{-1.710}$$
...
$$ L(k = 2\ |\ p) = \left( \binom{4}{2} \cdot p^2 \cdot (1-p)^2 \right)^{14} $$
$$ L(k = 3\ |\ p) = \left( \binom{4}{3} \cdot p^3 \cdot (1-p)^1 \right)^{7} $$
