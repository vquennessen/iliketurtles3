...
pop_dynamics <- function(N, max_age, y, M,
  ...
  if (evolution == TRUE) {
    ...
    G_new <- asplit(array(rep(NA, 4 * max_age * max_N),
                   dim = c(4, max_age, max_N)), 
                   c(1, 2))
    ...
    for (a in 2:max_age) {
      ...
      if (sum(!is.na(G_old_IF)) > 0 & size_sIF > 0) {
        ...
        if (size_nMF > 0) {
          ...
          # G_new[1, a] <- list(new_sIF[-mf_indices], 
          #                rep(NA, max_N - length(new_sIF[-mf_indices])))
          G_new[1, a] <- list(new_sIF[-mf_indices])
          
          # if none matured
        } else { 
          # G_new[1, a] <- list(new_sIF, 
      ...
      if (is.null(G_new_nMF) & is.null(G_new_sMF)) {
        # G_new[3, a][[1]] <- rep(NA, max_N)
        G_new[3, a] <- list(NA)
        
      } else {
        # G_new[3, a][[1]] <- c(G_new_nMF, G_new_sMF, 
      ...
      if (sum(!is.na(G_old_IM)) > 0 & size_sIM > 0) {
        ...
        if (size_nMM > 0) {
          ...
          # G_new[2, a][[1]] <- c(new_sIM[-mm_indices], 
          #                rep(NA, max_N - length(new_sIM[-mm_indices])))
          G_new[2, a] <- list(new_sIM[-mm_indices])
          
          # if none matured
        } else { 
          # G_new[2, a][[1]] <- c(new_sIM, 
          #                rep(NA, max_N - length(new_sIM)))
          G_new[2, a] <- list(new_sIM)
          G_new_nMM <- NULL 
        }
        
        # if there are no old immature genotypes to sample OR none survived
      } else {
        # G_new[2, a][[1]] <- rep(NA, max_N)
        G_new[2, a] <- list(NA)
      ...
      if (is.null(G_new_nMM) & is.null(G_new_sMM)) {
        # G_new[4, a][[1]] <- rep(NA, max_N)
        G_new[4, a] <- list(NA)
        
      } else {
        # G_new[4, a][[1]] <- c(G_new_nMM, G_new_sMM, 
    ...
    # G_stats[, , y, 2] <- lapply(G_new, c(1, 2), median, na.rm = TRUE)
    # G_stats[, , y, 3] <- lapply(G_new, c(1, 2), var, na.rm = TRUE)
    ...
    # P_new <- asplit(array(rnorm(n = c(4 * max_age * max_N),
    #                             mean = G_new,
