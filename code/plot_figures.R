    ...
    for (y in 1:Y) {
      ...
      if (file.exists(paste('C:/Users/Vic/Box Sync/Quennessen_Thesis/PhD Thesis/model output/',
        ...
        if (y > 1) {
          ...
          DF$Lambda[index] = log(DF$Population_size[index] / DF$Population_size[index - 1])
          
          # d Lambda / d time
          DF$dLdtemp[index] <- (DF$Lambda[index] - DF$Lambda[2]) / (scenarios[s]/100*y)
        ...
        if (y > 1) {
          ...
          DF$Lambda_mature[index] = log(DF$Population_mature[index] / DF$Population_mature[index - 1])
          
          # d Lambda mature / d time
          DF$dLMdtemp[index] <- (DF$Lambda_mature[index] - DF$Lambda_mature[2]) / (scenarios[s]/100*y)
...
for (i in 1:length(years_to_plot)) {
  ...
  # subset for year
  to_plot <- subset(DF, Year == years_to_plot[i])
  
  # heatmap for change in lambda for full population size
  fig_all <- ggplot(data = to_plot, aes(x = Beta, y = Scenario, fill = dLdtemp)) +
    ...
    ggtitle(paste('Change in total population growth rate by year ', 
                  years_to_plot[i], sep = '')) +
